...
%% Source fonts: https://ctan.org/tex-archive/fonts/cochineal/opentype
%%
...
\voffset=\dimexpr((\pageheight/9)-\topskip+1ex)

% Typeset area size
\hsize=\dimexpr((\pagewidth/9)*6)
\vsize=\topskip \advance\vsize by 26\baselineskip


%%%%%%%%% P A G E   A D J U S T M E N T %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\headline{%
	\ifheadline
		\ifodd\pageno
			\hfill{\normalsize\sc metamorphosis}\hfill% recto
		\else
			\hfill{\normalsize\sc franz kafka}\hfill% verso	
		\fi
	\else
		\hss
		\global\headlinetrue	
...
\def\makefootline{%
	\vskip0.5\baselineskip % adjust to move footline
	\lineskiplimit=0pt
	\line{%
		\the\footline
	}%
...
\footline{%
	\iffootline
		\ifodd\pageno
			\hss{\normalsize\rm \folio}\hskip\parindent% recto
		\else
			\hskip\parindent{\normalsize\rm \folio}\hss% verso	
		\fi
	\else
		\hss
		\global\footlinetrue	
...
\def\beginchapter#1{%
	\ifodd\pageno
	\else
		\null\thispagenofootline \thispagenoheadline
		\vfil\break
	\fi
	\thispagenoheadline
	\centerline{\chaptersize\rm #1}
	\vskip3\baselineskip
	\firstnoindent
}
...
\newcount\tmpcount
\newcount\signaturelength	\signaturelength=16 % Pages in a signature
\newdimen\oldhoffset

\def\adjustforcreep{%
	% Find current page place in signature
	\tmpcount=\pageno%
	\divide\tmpcount by\signaturelength%
	\multiply\tmpcount by\signaturelength%
	\multiply\tmpcount by-1%
	\advance\tmpcount by\pageno\relax%
	% Adjust based on place in signature
	\ifnum\tmpcount=1
	...
	\fi
	\ifnum\tmpcount=3
		\advance\hoffset by 1.5mm%
	\fi
	% Etc for all the pages in a signature
}
...
%\moveleft\hoffset
%\vbox to 0pt{\vskip-\topskip\vskip-\voffset{\image{canon136mm204mm-1.pdf}}}
%\vskip-\baselineskip

%%%%%%%%% C O N T E N T %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Begin Titlepage
\thispagenofootline \thispagenoheadline
\vglue3\baselineskip
\centerline{\titlesize\rm METAMORPHOSIS}
\vskip\baselineskip
\centerline{\chaptersize\rm by}			
\vskip\baselineskip
\centerline{\chaptersize\rm Franz Kafka}
\vfill
